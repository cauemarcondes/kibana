...
export function compressSpans({
  ...
  itemsToProcess.push(waterfall.entryWaterfallTransaction);
    ...
    if (nextItemToProcess) {
      ...
      if (children) {
        ...
        transformChildren({
          ...
          transformedItems,
          itemsToProcess,
          childrenByParentId: waterfall.childrenByParentId,
          durationThresholdMs,
          nPlusOneThreshold,
        });
      }
...
function transformChildren({
  ...
  transformedItems: IWaterfallItem[];
  itemsToProcess: IWaterfallItem[];
  childrenByParentId: Record<string | number, IWaterfallItem[]>;
  durationThresholdMs: number;
  nPlusOneThreshold: number;
}) {
  let prevItem: IWaterfallItem | undefined;

  const state: AlgorithmState = {
    compressedSpan: undefined,
    prevCompressedSpan: undefined,
  };
  const durationThreshold = durationThresholdMs * 1000;
  for (let i = 0; i < children.length; i++) {
    const item = children[i];
    const hasSubChildren = childrenByParentId[item.id] !== undefined;
    if (hasSubChildren || item.docType !== 'span') {
      // current item is a parent or not a span
...
function processExitSpan(
  transformedItems: IWaterfallItem[],
  item: IWaterfallSpan,
  prevItem: IWaterfallItem | undefined,
  state: AlgorithmState,
  durationThreshold: number,
  nPlusOneThreshold: number
) {
  if (prevItem && prevItem.docType !== 'span') {
    return;
  }
  const isShort = item.duration < durationThreshold;
  const sameDestination =
    state.compressedSpan &&
    item.doc.span.type === state.compressedSpan.type &&
    item.doc.span.subtype === state.compressedSpan.subtype &&
    item.doc.span.destination !== undefined &&
    item.doc.span.destination?.service.resource ===
      state.compressedSpan.destination;

  const sameQueryAsPrev =
    prevItem &&
    prevItem.doc.span.db &&
    sameDestination &&
    item.doc.span.db &&
    item.doc.span.db?.statement === prevItem.doc.span.db?.statement;

  const sameQueryAsCompressedSpan =
    sameDestination &&
    item.doc.span.db &&
    state.compressedSpan &&
    item.doc.span.db?.statement === state.compressedSpan.query;

  const cSpanContainsLongCall =
    state.compressedSpan &&
    state.compressedSpan.maxDuration >= durationThreshold;

  const cSpanIsNPlusOneCandidate =
    state.compressedSpan && state.compressedSpan.query;

  if (isShort && !sameQueryAsPrev) {
    // current span is short but with different query as previous
    if (
      sameDestination &&
      !cSpanContainsLongCall &&
      !cSpanIsNPlusOneCandidate
    ) {
    ...
    if (sameQueryAsCompressedSpan) {
      ...
      state.compressedSpan = newCSpan(prevItem!);
      extendCSpan(state, item);
    }
  } else {
...
function newCSpan(span: IWaterfallSpan): ICompressedSpan {
  return {
    count: 1,
    destination: span.doc.span.destination
      ? span.doc.span.destination.service.resource
      : span.doc.span.type +
        '/' +
        (span.doc.span.subtype ? span.doc.span.subtype : ''),
...
function reportCSpan(
  transformedItems: IWaterfallItem[],
  state: AlgorithmState,
  durationThreshold: number,
  nPlusOneThreshold: number
) {
  const { compressedSpan: cSpan, prevCompressedSpan: prevCSpan } = state;
  let itemToReport: IWaterfallSpan | undefined;
  ...
  if (!cSpan && prevCSpan) {
    ...
    const sameQuery =
      cSpan.query && prevCSpan.query && cSpan.query === prevCSpan.query;
    const bothShort =
      cSpan.maxDuration < durationThreshold &&
      prevCSpan.maxDuration < durationThreshold;
    const currentIsFullNPlusOne =
      cSpan.query && cSpan.count >= nPlusOneThreshold;
    const prevIsFullNPlusOne =
      prevCSpan.query && prevCSpan.count >= nPlusOneThreshold;
...
function getSpanItem(
  ...
  const originDoc = compressedSpan.originSpan.doc;
  let antiPattern = false;
  let newSpanName = originDoc.span.name;
  if (compressedSpan.query && compressedSpan.count >= nPlusOneThreshold) {
    newSpanName = '(' + compressedSpan.count + 'x) ' + originDoc.span.name;
    antiPattern = true;
  } else if (compressedSpan.query) {
    newSpanName = '(' + compressedSpan.count + 'x) ' + originDoc.span.name;
  } else {
    newSpanName =
      compressedSpan.count + 'x calls to ' + compressedSpan.destination;
  }
  const duration = compressedSpan.end - compressedSpan.start;
  const doc = newSpanDoc(
    originDoc,
    newSpanName,
    duration,
    compressedSpan.query
  );
...
function createWaterfall(
  ...
  modifiedWaterfall.antipatternDetected =
    items.find((item) => item.nPlusOne) !== undefined;
...
function getWaterfallItems(
  items: IWaterfallItem[]
): IWaterfallSpanOrTransaction[] {
  return items
    .filter((item) => item.docType === 'span' || item.docType === 'transaction')
